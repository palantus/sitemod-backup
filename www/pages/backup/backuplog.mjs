const elementName = 'backup-backup-log-page'
...
class Element extends HTMLElement {
  constructor() {
    ...
    this.backupId = /\/backup\/backup\/([\d]+)/.exec(state().path)[1]
  ...
  async refreshData(){
    let log = await api.get(`backup/backup/${this.backupId}/log`)
